...
class RunMangerLogicNovel():
    def __init__(self, model_config):
        self.database_manager = DatabaseManagerNovel(DB_PATH_LOGIC_NOVEL)
    ...
    def run_tests_and_collect_errors(self, code, entry_point, problem, model_name, dir, run_name, reprompt_info):
    ...
    def save_static_error_to_db(self, problem, model_name, dir, run_name, reprompt_info, static_error):
        test_loader = TestManager(problem, TEST_CASE_PATH)
        generated_tests = test_loader.load_test("generated.jsonl")
        official_tests = test_loader.load_test("official.jsonl")
    ...
    def run_evaluation_pipeline(self, problem, model_name, path, dir, run_name):

        pipeline_log_path = os.path.join(
            OUTPUT_PATH_LOGIC_NOVEL, model_name, run_name, problem, dir
        )
        ...
        logging.info(f"Evaluating {model_name}/{run_name}/{problem}/{dir}/ using {self.model_config['name']}")
        ...
        for i in range(MAX_REPROMPT + 1):
            ...
            if code != "code-not-found":
                ...
                if reprompt:
                    ...
                    self.save_static_error_to_db(problem, model_name, dir, run_name, f"reprompt_{nreprompt}", reprompt)
                    ...
                    if entry_point:
                        ...
                        runtime_errors = self.run_tests_and_collect_errors(
                            code, entry_point, problem, model_name, dir, run_name, f"reprompt_{nreprompt}"
                        )
                        ...
                        if i < MAX_REPROMPT:
                            ...
                            self.save_static_error_to_db(problem, model_name, dir, run_name, f"reprompt_{nreprompt}", REPROMPT_ENTRY_POINT_NOT_FOUND)
                ...
                if i < MAX_REPROMPT:
                    ...
                    self.save_static_error_to_db(problem, model_name, dir, run_name, f"reprompt_{nreprompt}", REPROMPT_CODE_NOT_FOUND)
            ...
            for i in range(N_RUN):
                if i > 2:
                    ...
                    for problem in self.dataset:
                        full_path = os.path.join(INPUT_PATH_LOGIC_NOVEL, model_name, f"run_{i+1}", problem)
                        
                        for root, dirs, files in os.walk(full_path):
                            for dir in dirs:
                                path = os.path.join(full_path, dir, f'00_reasoning.txt')
                                try:
                                    self.run_evaluation_pipeline(problem, model_name, path, dir, f"run_{i + 1}")
